...
console.log("Spatial Web Module Loaded (Main Thread RESTORED)");
...
function loadCursor() {
    ...
    loader.load('cursor.glb', (gltf) => {
        ...
        // Position and scale
        cursor.position.set(0, 0, -20); // 20 units in front of camera
        cursor.scale.set(1, 1, 1); // Use original scale

        // Keep original materials but ensure they render on top
        cursor.traverse((child) => {
            if (child.isMesh) {
                // Keep the original material, just adjust render settings
                if (child.material) {
        ...
        camera.add(cursor);
        console.log("3D Cursor loaded with original materials");
    }, undefined, (e) => {
...
export function transformTo3D() {
    ...
    if (cssRenderer) cssRenderer.domElement.style.display = 'block';
