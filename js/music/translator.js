...
let gl, program, positionBuffer, vao;
...
function initWebGLBackground(canvas) {
    ...
    const fsSource = `#version 300 es
        ...
        void main() {
            ...
            float d = (0.0 - ro.y) / rd.y;
            vec3 p = ro + rd * d;
            
            float lineIntensity = 0.0;
            if (d > 0.0 && d < 50.0) {
                p.z += time * 2.0;
                float wave = sin(p.x * 0.5 + time) * 0.2 * u_reactivity;
                p.y += wave;
                
                vec2 grid = abs(fract(p.xz * 0.5 - 0.5) - 0.5);
                float dist = min(grid.x, grid.y);
                lineIntensity = smoothstep(0.04, 0.0, dist) * exp(-d * 0.1);
            }
            ...
            for(int i=0; i<3; i++) {
                float fi = float(i);
                vec2 start = vec2(sin(fi*2.0)*0.5, -0.8);
                vec2 end = vec2(sin(fi*3.0 + time*0.2)*0.4, 0.4 + sin(fi)*0.2);
                float l = line(st, start, end);
                tree += smoothstep(0.02, 0.0, l) * 0.5;
                
                // Petals (Gaussian Splats)
                for(int j=0; j<5; j++) {
                    float fj = float(j);
                    vec2 pPos = mix(start, end, 0.6 + fj*0.1);
                    pPos += vec2(sin(time + fi + fj)*0.1, cos(time*0.8 + fi*fj)*0.05);
                    float pDist = length(st - pPos);
                    tree += exp(-pDist * 20.0) * (0.3 + u_bass * 0.5);
                }
            }

            vec3 color = mix(vec3(0.01, 0.02, 0.05), vec3(0.0, 1.0, 0.8), lineIntensity * (0.5 + u_reactivity*0.5));
            color += vec3(1.0, 0.4, 0.7) * tree;
            
            // Scanlines
            color *= 0.9 + 0.1 * sin(gl_FragCoord.y * 1.5 + u_time * 10.0);
...
function renderCurrentBackground(canvas, reactivity = 0, bass = 0) {
    ...
    gl.uniform1f(u_reactivity, reactivity);
