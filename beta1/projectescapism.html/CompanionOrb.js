...
class CompanionOrb {
    constructor(scene, playerRef, cameraRef, hivesRef) {
        ...
        // Behavior config
        this.followDistance = isEndgame ? 150.0 : 18.0;   // Lurks far behind — barely visible in darkness
        this.hoverHeight = isEndgame ? 85.0 : 15.0;     // Higher up for overview
        this.followSmooth = isEndgame ? 0.75 : 1.2;     // Slower response to hive switching
        this.bobAmount = isEndgame ? 16.0 : 1.2;        // More pronounced heaving
        this.bobSpeed = isEndgame ? 0.25 : 0.4;         // Slower breathing
        this.orbitDrift = isEndgame ? 115.0 : 8.0;       // Much wider orbit around hives

        // Orb config — grotesque proportions
        this.config = {
            tentacleCount: isEndgame ? 32 : 24,
            orbSize: isEndgame ? 30.0 : 1.0,
            tentacleLength: isEndgame ? 130.0 : 5.0,
            tentacleThickness: isEndgame ? 1.85 : 0.07,
            wriggleAmount: isEndgame ? 18.0 : 2.5,
            wriggleSpeed: isEndgame ? 2.2 : 4.0,
            segments: 32,
            radialSegments: 6
        };
    ...
    _buildTether() {
        const isEndgame = window.GAME_START_CONFIG && window.GAME_START_CONFIG.mapId === 'endgame';
        this.tetherRadius = isEndgame ? 1.35 : 0.04;
        ...
        const tetherGeo = new THREE.TubeGeometry(this.tetherCurve, 48, this.tetherRadius, 6, false);
    ...
    _buildPointLight() {
        const isEndgame = window.GAME_START_CONFIG && window.GAME_START_CONFIG.mapId === 'endgame';
        // Dim, sinister blood-red glow (or massive glowing beacon in endgame)
        this.light = new THREE.PointLight(0xff0000, isEndgame ? 4.5 : 0.8, isEndgame ? 500 : 12);
    ...
    _updateTether(delta, time) {
        ...
        for (let i = 0; i < numPoints; i++) {
            ...
            const sag = Math.sin(t * Math.PI) * (isEndgame ? 45.0 : 3.0);
            base.y -= sag;

            // More aggressive wriggle — sinew twitching
            base.x += Math.sin(time * 3.0 + t * 8.0) * (isEndgame ? 6.5 : 0.3) * Math.sin(t * Math.PI);
            base.z += Math.cos(time * 2.5 + t * 6.0) * (isEndgame ? 5.5 : 0.25) * Math.sin(t * Math.PI);
            base.y += Math.sin(time * 4.0 + t * 10.0) * (isEndgame ? 3.5 : 0.1) * Math.sin(t * Math.PI);
        ...
        this.tetherMesh.geometry = new THREE.TubeGeometry(this.tetherCurve, 48, this.tetherRadius || 0.035, 6, false);
        oldGeo.dispose();
    }
