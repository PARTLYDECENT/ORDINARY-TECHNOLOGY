        ...
        const mat = new THREE.ShaderMaterial({
            ...
            fragmentShader: `
                ...
                void main() {
                    ...
                    if (uIsEndgame > 0.5) {
                        zenithColor = vec3(0.005, 0.002, 0.015);
                        horizonColor = vec3(0.02, 0.005, 0.05);
                    }
                    ...
                    if (uIsEndgame > 0.5) {
                        ...
                        // 1. Double layer starry sky with colorful clusters
                        float starNoise = hash(viewDir * 700.0);
                        float starMask = smoothstep(0.9982, 1.0, starNoise);
                        float twinkle = sin(uTime * 3.5 + starNoise * 15.0) * 0.5 + 0.5;
                        
                        // Vary star color (white, hot cyan, cool amber)
                        vec3 starColor = vec3(1.0);
                        if (starNoise > 0.9992) starColor = vec3(0.4, 0.8, 1.0); // Cyan star
                        else if (starNoise < 0.9985) starColor = vec3(1.0, 0.7, 0.4); // Amber star
                        skyColor += starColor * starMask * twinkle * 1.5;

                        // 2. Cosmic Singularity (Black Hole at zenith)
                        vec3 singularityCenter = vec3(0.0, 1.0, 0.0);
                        float sDot = dot(viewDir, singularityCenter);
                        float gravitationalLensing = smoothstep(0.965, 0.985, sDot);
                        
                        // Event horizon shadow core
                        float eventHorizon = step(0.994, sDot);
                        
                        // Accretion disk rings
                        float ringDist = acos(sDot);
                        float ringPattern = sin(ringDist * 160.0 - uTime * 6.0) * 0.5 + 0.5;
                        float ringMask = smoothstep(0.12, 0.02, abs(ringDist - 0.08)) * (1.0 - eventHorizon);
                        vec3 ringColor = mix(vec3(0.0, 0.8, 1.0), vec3(1.0, 0.0, 0.8), ringPattern);
                        skyColor += ringColor * ringMask * 3.2;

                        // Add gravitational light bending around event horizon
                        skyColor = mix(skyColor, vec3(0.0), eventHorizon);

                        // 3. Volumetric Nebulae gas lanes (FBM noise)
                        vec3 nebPos1 = viewDir * 1.8 + vec3(uTime * 0.008, 0.0, uTime * 0.004);
                        vec3 nebPos2 = viewDir * 2.5 - vec3(0.0, uTime * 0.006, uTime * 0.005);
                        
                        float neb1 = fbm(nebPos1 * 1.6);
                        float neb2 = fbm(nebPos2 * 2.2);
                        
                        float gasDensity1 = smoothstep(0.35, 0.85, neb1);
                        float gasDensity2 = smoothstep(0.40, 0.90, neb2);
                        
                        vec3 gasColor1 = mix(vec3(0.5, 0.0, 0.8), vec3(0.0, 0.65, 0.9), sin(uTime * 0.15 + viewDir.x * 2.0) * 0.5 + 0.5);
                        vec3 gasColor2 = mix(vec3(0.9, 0.0, 0.45), vec3(0.0, 0.9, 0.45), cos(uTime * 0.22 - viewDir.z * 1.5) * 0.5 + 0.5);
                        
                        // Add nebulae glow to background
                        skyColor = mix(skyColor, skyColor + gasColor1 * 1.5, gasDensity1 * 0.7);
                        skyColor = mix(skyColor, skyColor + gasColor2 * 1.8, gasDensity2 * 0.55);

                        // 4. Dark Matter interstellar dust absorption lanes
                        float dustVal = fbm(viewDir * 3.0 + vec3(0.0, uTime * 0.002, 0.0));
                        float dustAbsorption = smoothstep(0.55, 0.95, dustVal);
                        skyColor = mix(skyColor, skyColor * 0.15, dustAbsorption * 0.8);
                        
                    } else {
                        // Standard sky mechanics
                        vec3 sunDir = normalize(uSunPos);
