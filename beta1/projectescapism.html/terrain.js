...
const TerrainGen = {
    ...
    getHeight: function (x, z) {
        ...
        if (window.GAME_START_CONFIG && (window.GAME_START_CONFIG.mapId === 'facility' || window.GAME_START_CONFIG.mapId === 'endgame' || window.GAME_START_CONFIG.mapId === 'asynchronousmaze1')) {
    ...
    getMeshHeight: function (x, z) {
        if (window.GAME_START_CONFIG && window.GAME_START_CONFIG.mapId === 'endgame') {
            ...
            let wave = Math.sin(x * 0.04 + t * 1.5) * Math.cos(z * 0.04 + t * 1.5) * 1.8;
            wave += Math.sin(x * 0.12 - t * 3.0) * Math.cos(z * 0.12 - t * 3.0) * 0.4;
            
            // Crimson Void spikes
            if (window.RealityPhaseShifter && window.RealityPhaseShifter.activeDimension === 1) {
                const spikes = Math.sin(x * 0.25) * Math.cos(z * 0.25) * 3.2;
                wave += spikes;
            }
            
            // Propagating shockwave ring
            if (window.mapManager && window.mapManager.endgamePulse > 0.01) {
                const pTime = window.mapManager.endgamePulseTime || 0;
                const dT = t - pTime;
                if (dT > 0.0 && dT < 3.0) {
                    const dx = x - px;
                    const dz = z - pz;
                    const distToCam = Math.sqrt(dx * dx + dz * dz);
                    const ringRadius = dT * 18.0;
                    const distToRing = Math.abs(distToCam - ringRadius);
                    const fade = Math.exp(-dT * 0.8) * window.mapManager.endgamePulse;
                    const ripple = Math.sin(distToRing * 0.8 - t * 12.0) * (distToRing < 8.0 ? (1.0 - distToRing / 8.0) : 0.0) * fade * 4.5;
                    wave += ripple;
                }
            }

            // Singularity sink zones displacement
            if (window.mapManager && window.mapManager.sinkZones) {
                for (let k = 0; k < window.mapManager.sinkZones.length; k++) {
                    const szone = window.mapManager.sinkZones[k];
                    const dx = x - szone.x;
                    const dz = z - szone.z;
                    const distSq = dx * dx + dz * dz;
                    const radSq = szone.radius * szone.radius;
                    if (distSq < radSq) {
                        const factor = 1.0 - Math.sqrt(distSq) / szone.radius;
                        const smoothFactor = factor * factor * (3.0 - 2.0 * factor);
                        wave += szone.depth * smoothFactor;
                    }
                }
            }
...
`;
        ...
        terrainMat.onBeforeCompile = (shader) => {
            ...
            shader.uniforms.uEndgamePulse = { value: 0.0 };
            shader.uniforms.uEndgamePulseTime = { value: 0.0 };
            shader.uniforms.uRealityShift = { value: 0.0 };
            ...
            shader.vertexShader = glslFBM + `
                ...
                uniform float uEndgamePulse;
                uniform float uEndgamePulseTime;
                uniform float uRealityShift;
                varying vec2 vWorldPosRaw;
                varying float vDistToCam;
                varying float vHeight;

                float getEndgameHeight(vec2 p) {
                    float wave = sin(p.x * 0.04 + uTime * 1.5) * cos(p.y * 0.04 + uTime * 1.5) * 1.8;
                    wave += sin(p.x * 0.12 - uTime * 3.0) * cos(p.y * 0.12 - uTime * 3.0) * 0.4;
                    float dToPlayer = length(p - uPlayerPos);
                    float camWell = sin(dToPlayer * 0.4 - uTime * 6.0) * exp(-dToPlayer * 0.07) * 1.5;
                    wave += camWell;
                    if (uRealityShift > 0.5) {
                        float spikes = sin(p.x * 0.25) * cos(p.y * 0.25) * 3.2;
                        wave += spikes;
                    }
                    float dT = uTime - uEndgamePulseTime;
                    if (dT > 0.0 && dT < 3.0) {
                        float ringRadius = dT * 18.0;
                        float distToRing = abs(dToPlayer - ringRadius);
                        float fade = exp(-dT * 0.8) * uEndgamePulse;
                        float ripple = sin(distToRing * 0.8 - uTime * 12.0) * smoothstep(8.0, 0.0, distToRing) * fade * 4.5;
                        wave += ripple;
                    }
                    return wave;
                }
            ...
            shader.vertexShader = shader.vertexShader.replace(
                ...
                `#include <beginnormal_vertex>
                 ...
                 float hL, hR, hD, hU;
                 if (uMapType == 5) {
                     hL = getEndgameHeight(p + vec2(-eps, 0.0));
                     hR = getEndgameHeight(p + vec2(eps, 0.0));
                     hD = getEndgameHeight(p + vec2(0.0, -eps));
                     hU = getEndgameHeight(p + vec2(0.0, eps));
                 } else {
                     hL = getTerrainHeight(p + vec2(-eps, 0.0));
                     hR = getTerrainHeight(p + vec2(eps, 0.0));
                     hD = getTerrainHeight(p + vec2(0.0, -eps));
                     hU = getTerrainHeight(p + vec2(0.0, eps));
                 }
            ...
            shader.vertexShader = shader.vertexShader.replace(
                ...
                `#include <begin_vertex>
                 ...
                 if (uMapType == 5) {
                     vHeight = getEndgameHeight(vWorldPosRaw);
                 }
            ...
            shader.fragmentShader = shader.fragmentShader.replace(
                ...
                if (uMapType == 7) {
                    ...
                    // 1. Cyber Grid Math (spaced 20.0 units)
                    vec2 gridUv = fract(vWorldPosRaw * 0.05);
                    
                    // Chromatic separation on grid lines
                    float gridR = smoothstep(0.022, 0.0, abs(gridUv.x - 0.5));
                    float gridG = smoothstep(0.018, 0.0, abs(gridUv.y - 0.5));
                    float gridB = smoothstep(0.015, 0.0, abs(gridUv.x - 0.5) * abs(gridUv.y - 0.5) - 0.25);
                    
                    float pulse = 0.75 + 0.25 * sin(uTime * 3.0 + vWorldPosRaw.x * 0.02);
                    
                    // 2. Base Dark Cyber Obsidian color
                    vec3 baseCol = vec3(0.004, 0.002, 0.01);
                    
                    // 3. Glowing neon colors
                    vec3 gridCol = vec3(0.0, 0.85, 0.95) * gridR * pulse; // Cyan horizontal
                    gridCol += vec3(0.75, 0.0, 0.9) * gridG * (1.0 - pulse * 0.2); // Purple vertical
                    gridCol += vec3(1.0, 0.0, 0.5) * gridB * 0.45; // Magenta intersections
                    
                    // 4. Glowing motherboard nodes
                    vec2 nodeCell = floor(vWorldPosRaw * 0.05);
                    float nodeHash = fract(sin(dot(nodeCell, vec2(12.9898, 78.233))) * 43758.5453);
                    float nodeFlash = smoothstep(0.97, 1.0, sin(uTime * (1.5 + nodeHash * 2.0) + nodeHash * 10.0));
                    float nodeDist = length(gridUv - vec2(0.5));
                    float nodeGlow = smoothstep(0.12, 0.0, nodeDist) * nodeFlash * 1.5;
                    vec3 nodeCol = vec3(0.0, 1.0, 0.8) * nodeGlow;
                    
                    // 5. Horizontal scanlines
                    float scanline = 0.88 + 0.12 * sin(vWorldPosRaw.y * 3.14 - uTime * 4.0);
                    
                    // 6. Height-based glow intensity
                    float heightGlow = clamp((vHeight + 2.0) / 4.0, 0.5, 1.5);
                    
                    diffuseColor.rgb = (baseCol + gridCol + nodeCol) * scanline * heightGlow;
