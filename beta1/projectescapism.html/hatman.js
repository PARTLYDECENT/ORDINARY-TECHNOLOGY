...
class Hatman {
    ...
    _createBossHealthBar() {
        ...
        ui.className = 'fixed top-8 left-1/2 -translate-x-1/2 w-[380px] bg-slate-950/80 border border-red-950/60 backdrop-blur-md px-4 py-2 rounded-md z-[999] flex flex-col gap-1 select-none pointer-events-none transition-opacity duration-500';
        ui.innerHTML = `
    ...
    update(playerPos, delta, activeCamera = null) {
        ...
        // Limit boss to desolation boundary range (60 units of obsidian map center)
        const arenaDistSq = this.homePosition.x * this.homePosition.x + this.homePosition.z * this.homePosition.z;
        if (arenaDistSq > 3600.0) {
            const dist = Math.sqrt(arenaDistSq);
            this.homePosition.x = (this.homePosition.x / dist) * 60.0;
            this.homePosition.z = (this.homePosition.z / dist) * 60.0;
        }
