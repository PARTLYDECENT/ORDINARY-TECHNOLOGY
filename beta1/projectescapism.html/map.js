            ...
            leaves.forEach(leaf => {
                ...
                for(let x=leaf.x + 1; x < leaf.x + leaf.w - 1; x++) {
                    for(let z=leaf.z + 1; z < leaf.z + leaf.h - 1; z++) {
                        const gh = TerrainGen.getHeight(x * this.config.cellSize + this.config.cellSize/2 + worldOffsetX, z * this.config.cellSize + this.config.cellSize/2 + worldOffsetZ);
                ...
                corners.forEach(c => {
                    ...
                    const gh = TerrainGen.getHeight(wx, wz);
        ...
        for(let i=0; i<numCells; i++) {
            ...
            if(costField[i] === 255) {
                ...
                if (Math.random() < 0.15 && wCount < 500) {
                    ...
                    const dgh = TerrainGen.getHeight(dx + worldOffsetX, dz + worldOffsetZ);
                ...
                if (rand < pTree && tCount < 300) { // Trees
                    ...
                    const gh = TerrainGen.getHeight(tx, tz);
                    ...
                    const gh = TerrainGen.getHeight(rx + worldOffsetX, rz + worldOffsetZ);
