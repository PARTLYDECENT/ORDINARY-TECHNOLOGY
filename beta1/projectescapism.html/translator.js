...
const TerrainGen = {
    ...
    getHeight: function (x, z) {
        ...
        if (currentMapId === 'endgame') {
            const t = this.time || 0;
            const px = (this.playerPos && this.playerPos.x) || 0;
            const pz = (this.playerPos && this.playerPos.z) || 0;
            let wave = Math.sin(x * 0.04 + t * 1.5) * Math.cos(z * 0.04 + t * 1.5) * 1.8;
            wave += Math.sin(x * 0.12 - t * 3.0) * Math.cos(z * 0.12 - t * 3.0) * 0.4;
            
            // Crimson Void spikes
            if (this.activeDimension === 1) {
                wave += Math.sin(x * 0.25) * Math.cos(z * 0.25) * 3.2;
            }
            
            // Propagating shockwave ring
            if (this.endgamePulse > 0.01) {
                const dT = t - this.endgamePulseTime;
                if (dT > 0.0 && dT < 3.0) {
                    const dx = x - px;
                    const dz = z - pz;
                    ...
                    const fade = Math.exp(-dT * 0.8) * this.endgamePulse;
...
self.onmessage = function (e) {
    ...
    // Sync to TerrainGen for wave math in worker
    TerrainGen.time = elapsedTime;
    TerrainGen.activeDimension = config.activeDimension || 0;
    TerrainGen.endgamePulse = config.endgamePulse || 0.0;
    ...
    if (!self.prevZState) {
        ...
        self.zFallY = new Float32Array(config.maxZombies || 250);
    }

    for (let j = 0; j < data.spawnedZombies; j++) {
        if (zState[j] !== 0 && self.prevZState[j] === 0) {
            if (currentMapId === 'endgame') {
                self.zFallY[j] = 38.0; // starts in sky (drips from massive hive at 38)
            } else {
                self.zFallY[j] = 0.0;
            }
        }
    ...
    for (let i = 0; i < data.spawnedZombies; i++) {
        ...
        let zh = TerrainGen.getMeshHeight(zx, zz);
        if (currentMapId === 'endgame' && self.zFallY) {
            if (self.zFallY[i] > 0.0) {
                self.zFallY[i] = Math.max(0.0, self.zFallY[i] - delta * 20.0);
                if (self.zFallY[i] === 0.0) {
                    triggerAudio.push({ type: 'SPLASH_LANDING', x: zx, z: zz });
                }
            }
            zh += self.zFallY[i];
        }
